...
describe("Basic Sorting Algorithms", function () {
  ...
  it("performs a out-of-place insertion sort", function () {
    ...
    expect(consoleSpy).on.nth(1).be.called.with("9");
    expect(consoleSpy).on.nth(2).be.called.with("7,9");
    expect(consoleSpy).on.nth(3).be.called.with("5,7,9");
    expect(consoleSpy).on.nth(4).be.called.with("3,5,7,9");
    expect(consoleSpy).on.nth(5).be.called.with("1,3,5,7,9");
    expect(consoleSpy).on.nth(6).be.called.with("1,3,5,7,8,9");
    expect(consoleSpy).on.nth(7).be.called.with("1,3,5,6,7,8,9");
    expect(consoleSpy).on.nth(8).be.called.with("1,3,4,5,6,7,8,9");
